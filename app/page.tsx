...
type CleanupMode = "translate" | "fix" | "formal" | "bullet" | "email" | "slack";
type HistoryItem = {
  id: string;
  raw: string;
  cleaned: string | null;
...
const MODES: { value: CleanupMode; label: string; icon: string; color: string }[] = [
  { value: "translate", label: "Translate → EN", icon: "🌍", color: "bg-emerald-900/60 text-emerald-400 hover:bg-emerald-700 hover:text-white" },
  { value: "fix",       label: "Fix Grammar",    icon: "✏️", color: "bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white" },
  { value: "formal",    label: "Formal",          icon: "👔", color: "bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white" },
  { value: "bullet",    label: "Bullets",         icon: "•",  color: "bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white" },
  { value: "email",     label: "Email",           icon: "✉️", color: "bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white" },
  { value: "slack",     label: "Slack",           icon: "💬", color: "bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-white" },
];
...
export default function Home() {
  ...
  const scale = recording ? 1 + amplitude * 0.4 : 1;

  const HistoryPanel = (
    <div className="flex flex-col h-full">
      <div className="p-4 border-b border-gray-800 flex items-center justify-between">
        <h2 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">History</h2>
        <button onClick={() => setShowHistory(false)} className="md:hidden text-gray-600 hover:text-white text-xl">×</button>
      </div>
      ...
      {history.length > 0 && (
        <button onClick={() => { setHistory([]); localStorage.removeItem("whisperflow_history"); }}
    ...
    <div className="min-h-screen bg-gray-950 text-white flex">
      ...
      {showHistory && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-72 bg-gray-900 flex flex-col">{HistoryPanel}</div>
        </div>
      )}
      ...
      <main className="flex-1 flex flex-col items-center px-4 py-6 md:p-8 overflow-y-auto">
        ...
        <div className="w-full max-w-2xl mb-6 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => setShowHistory(true)} className="md:hidden flex flex-col gap-1 p-2 rounded-lg hover:bg-gray-800">
              <span className="block w-5 h-0.5 bg-gray-400" />
              <span className="block w-5 h-0.5 bg-gray-400" />
              <span className="block w-5 h-0.5 bg-gray-400" />
            </button>
            <div>
              <h1 className="text-xl md:text-2xl font-bold bg-gradient-to-r from-blue-400 to-violet-500 bg-clip-text text-transparent">WhisperFlow</h1>
              <p className="text-gray-600 text-xs mt-0.5">Bengali · Hindi · English · Mixed</p>
            </div>
          </div>
          <span className="text-xs bg-violet-900/50 text-violet-400 px-2 py-1 rounded-full border border-violet-800/50">⚡ Groq Whisper</span>
        </div>
        ...
        <div className="relative flex items-center justify-center mb-4">
          {recording && (
            <>
              <div className="absolute w-52 h-52 md:w-44 md:h-44 rounded-full bg-red-500/10 animate-ping" style={{ animationDuration: "1.5s" }} />
              <div className="absolute w-44 h-44 md:w-36 md:h-36 rounded-full bg-red-500/15 animate-pulse" />
            </>
          )}
          <button
            ...
            className={`relative w-36 h-36 md:w-28 md:h-28 rounded-full flex items-center justify-center select-none cursor-pointer ${
              recording
                ? "bg-red-500 shadow-[0_0_60px_rgba(239,68,68,0.6)]"
                : "bg-blue-600 hover:bg-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.4)] active:scale-95"
            }`}
          >
            {recording
              ? <div className="w-10 h-10 md:w-8 md:h-8 rounded-sm bg-white" />
              : <svg className="w-14 h-14 md:w-10 md:h-10 text-white" fill="currentColor" viewBox="0 0 24 24">
        ...
        <p className="text-gray-600 text-xs mb-5 text-center">
          {recording ? "" : <span>Hold button to record · <kbd className="bg-gray-800 px-1 rounded font-mono">Space</kbd> on desktop</span>}
        </p>

        {/* Status */}
        <div className="min-h-6 mb-4 text-center">
          {recording    && <p className="text-red-400 text-sm animate-pulse font-medium">Recording… release to transcribe</p>}
          {transcribing && <p className="text-yellow-400 text-sm animate-pulse font-medium">Transcribing… (auto-detects language)</p>}
          {cleaning     && <p className="text-blue-400 text-sm animate-pulse font-medium">Processing with AI…</p>}
          {error        && <p className="text-red-400 text-xs px-4 text-center">{error}</p>}
        </div>

        {/* AI Enhance buttons — always visible, disabled until there's text */}
          ...
          <div className="w-full max-w-2xl space-y-3">
            <div className="bg-gray-900 border border-gray-800 rounded-xl p-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => copy(rawText)} className="text-xs text-gray-600 hover:text-white transition-colors">Copy</button>
              </div>
              <p className="text-gray-200 leading-relaxed whitespace-pre-wrap text-sm md:text-base">{rawText}</p>
            </div>
